...
- Firebase Authentication, Firestore incident sync, and Storage integration when configured
- Demo mode with local browser storage and AI fallbacks for development without cloud credentials
...
The server optionally uses `GEMINI_API_KEY`, `GEMINI_MODEL` (defaults to `gemini-2.5-flash`), and `PORT` (defaults to `3000`). Use an empty `GEMINI_API_KEY` for the local demo fallback. See `.env.example` for the complete variable list.
...
Configure Firebase Authentication, Firestore, and Storage in the Firebase console, then apply the checked-in `firestore.rules`, `firestore.indexes.json`, and `storage.rules` as appropriate for the target project. Without Firebase configuration, CivicLens runs in its browser-local demo mode.
